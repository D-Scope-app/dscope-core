...
contract Survey {
    ...
    address public gate; // <-- NEW: eligibility gate (EIP-712 attester)
    ...
    // --- NEW: anti-replay for zkpass-lite
    mapping(bytes32 => bool) public nullifierUsed;

    // --- Prize-pool events
    event PrizeFunded(address indexed funder, uint256 amount); // name 'funder' to match indexer
    event PrizeSwept(address indexed to, uint256 amount);

    // --- Events (existing; KEEP NAMES for indexer)
    ...
    /// @notice constructor is payable to accept optional initial prize funding.
    constructor(
        ...
        creator = _creator;
        gate = _gate;               // may be address(0) for non-gated legacy surveys
        surveyType = _surveyType;
    ...
    /// @notice Legacy path — доступен ТОЛЬКО если gate == address(0).
    function vote(uint[][] calldata selectedOptionsPerQuestion) external {
        require(gate == address(0), "GATED_SURVEY"); // prevent bypass for gated surveys
        _voteCommon(selectedOptionsPerQuestion);
    }

    // ===== Voting with zkpass-lite proof =====
    /// @notice New path — требует настроенный gate и валидную подпись аттестатора.
    ...
    // ===== Shared vote logic (tallies + storage) =====
